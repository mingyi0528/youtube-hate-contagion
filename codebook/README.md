...
| File | Description |
|---|---|
| `codebook.py` | The exact system-instruction text (prompt) passed to the LLM classifier, plus the shared slur and trigger-word lexicons it references. This is the operational definition of both constructs as actually implemented, not a paraphrase of it. |

## What's in `codebook.py`
